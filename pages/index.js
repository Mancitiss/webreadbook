...
export default function Home() {
  ...
  async function getCategory() {
    let data_new = []
    await axios.get("/api/categorys/?format=json")
      .then(
        response => {
  ...
  async function getStory() {
    ...
    let us_temp = false
    let res = await axios.get("/api/list-story-new/")
      .then(
        response => {
          let data = response.data
          for (let temp of data) {
  ...
  return (
    <GoogleOAuthProvider clientId="786308916007-cm24heo3ecdro7amptnhe81shi0aoet2.apps.googleusercontent.com">
      <div >
        <Main>
          <Baner />
          ...
          <div className='home__silde'>
            ...
            <div className='home__silde__container' >

              {
                story.map((story) => {
                  return (
                    <Card key={story.id} index={story.id} story_name={story.story_name} image={story.image} total_chapters={story.total_chapters} introduce={story.introduce} owner={story.user} />)
                })
              }
            </div>
          </div>
                  ...
                  <div className='home__contaner__content--listfile' onClick={getCaStory}>
                    {
                      category.map((categori) => {
                        return (
                          <File key={categori.id} index={categori.id} name={categori.category_name} />)
                      })
                    }
                  </div>
                  <Row align='middle'>
                  ...
                  <Row>
                    {
                      caStory.map((caStory) => {
                        return (
                          <Card key={caStory.id} index={caStory.id} story_name={caStory.story_name} image={caStory.image} total_chapters={caStory.total_chapters} introduce={caStory.introduce} owner={caStory.user} />)
                      })
                    }
                  </Row>
                </div>
